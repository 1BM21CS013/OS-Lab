...
{    
    ...
    i = 0;
    while (currTime <= process[n - 1].at || !isEmpty(&sjfQueue) || currProcess)
    {
        ...
        if (!currProcess && !isEmpty(&sjfQueue))
            currProcess = pop(&sjfQueue);

        if (currProcess && currProcess->rt <= 0)
        {
            // note down turn around time
